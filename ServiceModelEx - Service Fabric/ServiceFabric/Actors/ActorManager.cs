...
namespace ServiceModelEx.ServiceFabric.Actors
{
   ...
   class ActorIdComparer : IEqualityComparer<ActorId>
   {
      ...
      {
         return x.ApplicationName.Equals(y.ApplicationName) && x.ActorServiceName.Equals(y.ActorServiceName) && x.ToString().Equals(y.ToString());
      }
      public int GetHashCode(ActorId obj)
      {
         return (obj.ApplicationName + obj.ActorServiceName + obj.ToString()).GetHashCode();
      }
   }
   ...
   internal static class ActorManager
   {
      ...
      static void Deactivate(KeyValuePair<ActorId,ActorInfo> pair,ConcurrentDictionary<ActorId,ActorInfo> instances)
      {
         ...
         ChannelFactory<IActor> factory = initializeInstance.Invoke(null,new object[] {null,new ProxyMessageInterceptor(pair.Value.ActorId,new ServiceContext(),null),ActorProxy.ProxyBinding,ActorProxy.ActorBinding}) as ChannelFactory<IActor>;
         IActor proxy = new ActorChannelInvoker<IActor>().Install(factory,pair.Value.ActorId,null);
