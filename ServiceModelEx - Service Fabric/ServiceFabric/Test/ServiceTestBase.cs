   ...
   public abstract class ServiceTestBase
   {
      ...
      static ServiceTestBase()
      {
         ...
         m_RegisterActorAsyncDefinition = typeof(ActorRuntime).GetMethod("RegisterActorAsync",
                                                                         ...
                                                                         null,
                                                                         new Type[] {typeof(Func<StatefulServiceContext,ActorTypeInformation,ActorService>),typeof(TimeSpan),typeof(CancellationToken)},
                                                                         null).GetGenericMethodDefinition();
      ...
      //Create mock message to hold headers during POCO testing.
      protected void MockMessage()
      {
         ...
         if(request.GetValue(OperationContext.Current) == null)
         {
      ...
      protected void RegisterMocks(params object[] mocks)
      {
         ...
         {
            if (mock.GetType().GetCustomAttribute(typeof(DataContractAttribute)) == null)
            {
               bool isActor = false;
               ...
               else
               {
                  ...
                  Type serviceType = ServiceMocks.Keys.FirstOrDefault(service=>service.GetInterfaces().Any(hasContract));
      ...
      [MethodImpl(MethodImplOptions.Synchronized)]
      public void Setup(Func<StatefulServiceContext,ActorTypeInformation,ActorService> actorServiceFactory,params Type[] types)
      {
         ...
         foreach(Type type in types)
         {
            if(type.IsSubclassOf(typeof(ActorBase)))
            {
               ...
               Task result = registerActor.Invoke(null,new object[] {actorServiceFactory,default(TimeSpan),default(CancellationToken)}) as Task;
               result.Wait();
            }
      ...
      //Cannot use header flow policy or flow interceptors here. May or may not have a stack to add the behavior to.
      protected virtual void AddStandardContexts(MessageHeaders headers)
      {}
      protected abstract void AddCustomContexts(bool isService,params object[] mocks);
      ...
      void MockActorContexts<I>(ActorBase target)
      {
         ...
         if(target == null)
         {
            ...
            ServiceContextHelper.Add(OperationContext.Current.OutgoingMessageHeaders,new ServiceContext { ServiceName = actorAddress });
            AddStandardContexts(OperationContext.Current.OutgoingMessageHeaders);
         }
         else
         {
            ...
            ServiceContextHelper.Add(OperationContext.Current.IncomingMessageHeaders,new ServiceContext { ServiceName = actorAddress });
            AddStandardContexts(OperationContext.Current.IncomingMessageHeaders);
         }
      }
         ...
         try
         {
            ...
            using(channel)
            {
               using(OperationContextScope scope = new OperationContextScope(channel))
               {
                  ...
                  if(callee is IActor)
                  {
                     isService = (callee is ActorBase) == false;
                     MockActorContexts<I>(callee as ActorBase);
                  ...
                  {
                     isService = (callee is StatelessService) == false;
                     MockServiceContext<I>(callee as StatelessService);
                  }

                  RegisterMocks(mocks);
                  AddCustomContexts(isService,mocks);
                                                                                                        ...
                                                                                                        where S : class,new() 
      {
         ...
         if(targetType.IsSubclassOf(typeof(ActorBase)))
         {
            ...
            poco = Activator.CreateInstance(targetType,new ActorService(context,ActorTypeInformation.Get(targetType)),new ActorId("Test")) as I;
         }
